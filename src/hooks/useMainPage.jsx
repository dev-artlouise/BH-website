...
export const useProjectContent = () => {
  ...
  return useQuery({
    ...
    queryFn: () => fetchProjectContent(), // Pass token to fetch function
    //enabled: !!token, // Only fetch if token is available
    retry: false, // Optional: Disable automatic retries if desired
    onError: (error) => {
      console.error("Error fetching projects data:", error.message);
...
export const useTestimonial = () => {
  ...
  return useQuery({
    queryKey: ["testimonialData"], // Unique key for the query
    queryFn: () => fetchTestimonial(), // Pass token to fetch function
    //enabled: !!token, // Only fetch if token is available
    retry: false, // Optional: Disable automatic retries if desired
    onError: (error) => {
      console.error("Error fetching projects data:", error.message);
...
export const useTestimonials = () => {
  ...
  return useQuery({
    queryKey: ["testimonialsData"], // Unique key for the query
    queryFn: () => fetchTestimonials(), // Pass token to fetch function
    //enabled: !!token, // Only fetch if token is available
    retry: false, // Optional: Disable automatic retries if desired
    onError: (error) => {
      console.error("Error fetching projects data:", error.message);
